  ...
  input.furniture.filter((item) => !targetFloors || targetFloors.has(item.floorId)).forEach((furniture) => {
    ...
    getFurnitureDemands(furniture).forEach((demand, index) => {
      ...
      const desired: DrawingItem = {
        ...
        electricalClass: demand.category === "network" ? "weak" : demand.category === "socket" ? "strong" : null,
        dedicatedCircuit: Boolean(demand.circuitId),
        waterSupplyKind: demand.category === "waterSupply" ? demand.type : null,
        drainageKind: demand.category === "drainage" ? demand.type : null,
