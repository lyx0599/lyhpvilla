...
for (const channel of ["normalMap", "roughnessMap", "aoMap"]) {
  ...
  assert.ok(furnitureMaterialSource.includes(channel), `家具材质未消费 ${channel}`);
}
assert.ok(furnitureMaterialSource.includes("meshPhysicalMaterial"), "玻璃应使用物理材质");
assert.ok(furnitureMaterialSource.includes("transmission"), "玻璃应支持透射");
