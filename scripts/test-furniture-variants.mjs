...
assert.equal(workspace.furniture.some((item) => item.id === "furn-kitchen-entry-island-001"), false, "The living-room island must remain removed from the default workspace.");
...
assert.equal(diningSet.render3d.variantId, "poweredRoundExtension");
assert.equal(diningSet.render3d.detailLevel, "presentation");
assert.equal(diningSet.render3d.primaryMaterial, "warmOak");
assert.equal(diningSet.render3d.secondaryMaterial, "beigeFabric");
assert.equal(diningSet.render3d.poweredTurntableDiameterCm, 75);
assert.equal(diningSet.render3d.turntableMount, "flushIntegrated");
const livingSofa = workspace.furniture.find((item) => item.id === "furn-1f-living-main-sofa-001");
assert.equal(livingSofa.render3d.variantId, "lowCurvedSofa");
assert.equal(livingSofa.render3d.primaryMaterial, "beigeFabric");
assert.equal(workspace.furniture.some((item) => item.id === "furn-1f-living-coffee-table-flex-001"), false);
const entryHangingRail = workspace.furniture.find((item) => item.id === "furn-entry-slim-hanging-001");
assert.equal(entryHangingRail.render3d.variantId, "slimHangingRail");
assert.equal(entryHangingRail.render3d.cabinetVisual.drawerCount, 0);
const kitchenRightRun = workspace.furniture.find((item) => item.id === "furn-kitchen-u-right-run");
assert.equal(kitchenRightRun.render3d.kitchenVisual.showUpperCabinets, true);
assert.equal(kitchenRightRun.render3d.kitchenVisual.upperCabinetHeightMm, 680);
assert.equal(kitchenRightRun.render3d.kitchenVisual.upperCabinetDepthMm, 350);
const bedroomWardrobe = workspace.furniture.find((item) => item.id === "module-1f-wardrobe-001");
assert.equal(bedroomWardrobe.render3d.variantId, "boutiqueOpenValetRack");
assert.equal(bedroomWardrobe.render3d.cabinetVisual.handleStyle, "edgePull");
const livingMediaWall = workspace.furniture.find((item) => item.id === "furn-living-fireplace-south-001");
assert.equal(livingMediaWall.render3d.variantId, "dualNicheMediaWall");
...
assert.match(rendererSources["cabinet-family-3d.tsx"], /integratedTaupeWardrobe/);
...
assert.match(rendererSources["media-wall-3d.tsx"], /floating-niche-media-wall/);
...
assert.equal(workspace.furniture.find((item) => item.id === "furn-2f-master-bedroom-large-wardrobe-001")?.dimensions.width, 300);
...
assert.equal(workspace.furniture.find((item) => item.id === "furn-b2-living-coffee-table-001")?.render3d?.variantId, "b2ModernNestedCoffeeTable");
assert.equal(workspace.furniture.find((item) => item.id === "furn-b2-living-long-sofa-001")?.render3d?.variantId, "b2FourSeatLeather");
assert.equal(workspace.furniture.find((item) => item.id === "furn-b2-entry-slim-foyer-cabinet-001")?.render3d?.variantId, "b2OpenFoyerRack");
assert.deepEqual(workspace.furniture.find((item) => item.id === "furn-b2-study-slab-table-001")?.dimensions, { width: 240, depth: 90, height: 75, unit: "cm" });
assert.equal(workspace.furniture.find((item) => item.id === "furn-b2-study-slab-table-001")?.render3d?.seatCount, 6);
assert.equal(workspace.furniture.find((item) => item.id === "furn-b2-under-stair-room-shell-001")?.render3d?.variantId, "b2UnderStairRoomShell");
