...
export function SofaFamily3D(props: FurnitureFamily3DProps) {
  ...
  const pillowCount = asset.detailLevel === "presentation" ? (resolved.variation.cushionBias > 0.6 ? 4 : 3) : asset.detailLevel === "standard" ? 2 : 1;
  ...
  return (
    <group>
      ...
      {Array.from({ length: pillowCount }, (_, index) => {
        ...
        const x = side * width * (0.19 + Math.floor(index / 2) * 0.12) + resolved.variation.asymmetry * 0.055;
        return (
          <RoundedPart
            key={index}
            size={[Math.min(0.42, width * 0.15), deep ? 0.34 : 0.3, 0.115]}
            position={[x, seatY + (deep ? 0.29 : 0.26) + (index % 2) * 0.025, -depth * 0.19]}
            rotation={[0.02, side * 0.05, side * (index % 2 ? -0.12 : 0.09)]}
            radius={0.07}
            detailLevel={asset.detailLevel}
            material={index === pillowCount - 1 ? asset.materials.accent : asset.materials.secondary}
            role="fabric"
            repeat={[1.5, 1.5]}
          />
        );
